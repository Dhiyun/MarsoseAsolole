<section id="contact" class="contact">

    <div class="container" data-aos="fade-up">

        <header class="section-header">
            <h2>Kontak</h2>
            <p>Hubungi Kita</p>
        </header>

        <div class="row gy-4">
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="300">
                <div
                    class="info-box d-flex flex-column justify-content-center align-items-center shadow-sm p-3 mb-5 bg-body rounded">
                    <i class="bi bi-telephone"></i>
                    <h3>Kontak Whatsapp</h3>
                    <p>+62 877 7046 7527<br>+62 851 0155 2315</p>
                </div>
                <div
                    class="info-box mt-4 d-flex flex-column justify-content-center align-items-center shadow-sm p-3 mb-5 bg-body rounded">
                    <i class="bi bi-clock"></i>
                    <h3>Jam Kerja RT/RW</h3>
                    <p>Setiap Hari</p>
                    <p>7:00AM - 05:00PM</p>
                </div>
            </div>
            <div class="col-md-8 shadow-sm p-3 mb-5 bg-body rounded" data-aos="fade-up" data-aos-delay="300">
                <iframe
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3951.2218203887423!2d112.64057307937847!3d-7.976011271548345!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2dd628474ea2985f%3A0x1a1513478579a578!2sJl.%20Marsose%2C%20Kesatrian%2C%20Kec.%20Blimbing%2C%20Kota%20Malang%2C%20Jawa%20Timur!5e0!3m2!1sen!2sid!4v1716035912149!5m2!1sen!2sid"
                    frameborder="0" style="border:0; width: 100%; height: 100%;" allowfullscreen="" loading="lazy"
                    referrerpolicy="no-referrer-when-downgrade"></iframe>
            </div>
        </div>


    </div>

</section><!-- End Contact Section -->
