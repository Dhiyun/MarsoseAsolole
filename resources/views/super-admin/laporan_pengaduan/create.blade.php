        ...
        <div class="modal-content">
            ...
            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                ...
                <form method="POST" id="kt_modal_add_laporan_form" class="form" action="{{ route('laporan.store') }}"
                    ...
                    @csrf
                    <!--begin::Scroll-->
                    <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_laporan_scroll"
                        data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}"
                        data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_laporan_header"
                        data-kt-scroll-wrappers="#kt_modal_add_laporan_scroll" data-kt-scroll-offset="300px">
                        <!-- Judul -->
                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2" for="judul">Judul</label>
                            <input type="text" id="judul" name="judul"
                                class="form-control form-control-solid mb-3 mb-lg-0" value="{{ old('judul') }}"
                                required />
                        </div>
                        <!-- Jenis Laporan -->
                        <div class="fv-row mb-7">
                            ...
                            <select id="jenis_laporan" name="jenis_laporan"
                                ...
                                <option value="Infrastruktur"
                                    {{ old('jenis_laporan') == 'Infrastruktur' ? 'selected' : '' }}>Infrastruktur
                                </option>
                                <option value="Keamanan" {{ old('jenis_laporan') == 'Keamanan' ? 'selected' : '' }}>
                                    Keamanan</option>
                                <option value="Kesehatan" {{ old('jenis_laporan') == 'Kesehatan' ? 'selected' : '' }}>
                                    Kesehatan</option>
                                <option value="Lingkungan" {{ old('jenis_laporan') == 'Lingkungan' ? 'selected' : '' }}>
                                    Lingkungan</option>
                                <option value="Layanan Masyarakat"
                                    {{ old('jenis_laporan') == 'Layanan Masyarakat' ? 'selected' : '' }}>Layanan
                                    Masyarakat</option>
                            </select>
                        </div>
                        <!--begin::Input group-->
