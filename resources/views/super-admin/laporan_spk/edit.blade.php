        ...
        <div class="modal-content">
            ...
            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                ...
                <form method="POST" id="kt_modal_edit_laporan_spk_form" class="form"
                    action="{{ route('laporan_spk.update', ['id' => $laporan->id_spk]) }}">
                    @csrf
                    @method('PUT')
                    <!--begin::Scroll-->
                    <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_edit_laporan_spk_scroll"
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <select class="form-select" id="jenis_laporan" name="jenis_laporan" required>
                                <option value="Infrastruktur"
                                    {{ $laporan->jenis_laporan == 'Infrastruktur' ? 'selected' : '' }}>Infrastruktur
                                </option>
                                <option value="Keamanan" {{ $laporan->jenis_laporan == 'Keamanan' ? 'selected' : '' }}>
                                    Keamanan</option>
                                <option value="Kesehatan"
                                    {{ $laporan->jenis_laporan == 'Kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                                <option value="Lingkungan"
                                    {{ $laporan->jenis_laporan == 'Lingkungan' ? 'selected' : '' }}>Lingkungan</option>
                                <option value="Layanan Masyarakat"
                                    {{ $laporan->jenis_laporan == 'Layanan Masyarakat' ? 'selected' : '' }}>Layanan
                                    Masyarakat</option>
                            </select>
                        </div>
                        <!-- Biaya -->
                            ...
                            <div class="form-check form-check-inline">
                                <div class="d-flex justify-content-between">
                                    <div class="form-check form-check-inline flex-grow-1 text-center">
                                        <input class="form-check-input" type="radio" name="dampak" id="dampak1"
                                            value="1" {{ $laporan->dampak == 1 ? 'checked' : '' }} required>
                                        <label class="form-check-label" for="dampak1">Rendah</label>
                                        <div id="durasiHelp1" class="form-text">1-50 Orang</div>
                                    </div>
                                    <div class="form-check form-check-inline flex-grow-1 text-center">
                                        <input class="form-check-input" type="radio" name="dampak" id="dampak2"
                                            value="2" {{ $laporan->dampak == 2 ? 'checked' : '' }} required>
                                        <label class="form-check-label" for="dampak2">Medium</label>
                                        <div id="durasiHelp1" class="form-text">1-50 Orang</div>
                                    </div>
                                    <div class="form-check form-check-inline flex-grow-1 text-center">
                                        <input class="form-check-input" type="radio" name="dampak" id="dampak3"
                                            value="3" {{ $laporan->dampak == 3 ? 'checked' : '' }} required>
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <input type="number" class="form-control" id="durasi_pekerjaan" name="durasi_pekerjaan"
                                value="{{ $laporan->durasi_pekerjaan }}" required>
                            <div id="durasiHelp" class="form-text">Silahkan masukkan per-hari</div>
                        </div>
                        <!-- SDM -->
                        <div class="fv-row mb-7">
                            <label for="sdm" class="form-label">SDM</label>
                            <input type="number" class="form-control" id="sdm" name="sdm"
                                value="{{ $laporan->sdm }}" required>
                            <div id="jumlah_pengaduanHelp" class="form-text">Silahkan masukkan jumlah orang yang
                                bekerja</div>
                        </div>
                    </div>
                    <!--end::Scroll-->
